...
const postController: { [key: string]: RequestHandler } = {
  paginateFeed: async (req, res) => {
    ...
    const feedPosts = await PostService.paginatePosts(
      {},
      { _id: -1 },
      anchor,
      prev
    ...
    if (userId) {
      await Promise.all(
        feedPosts.map((post: IPostDoc) => post.populateUserActions(userId))
      );
    }
  ...
  paginateReplies: async (req, res) => {
    ...
    if (postId) {
      const replies = await PostService.paginatePosts(
        { replyTo: postId },
        { _id: -1 },
        anchor,
        prev
      ...
      if (userId) {
        await Promise.all(
          replies.map((post: IPostDoc) => post.populateUserActions(userId))
        );
      }
  ...
  paginateProfile: async (req, res) => {
    ...
    if (!profileId && !username)
      throw new Error("Profile: Invalid request arguments");
    if (!profileId && username) {
    ...
    const profilePosts = await PostService.paginatePosts(
      { user: profileId },
      { _id: -1 },
      anchor,
      prev
    ...
    if (userId) {
      await Promise.all(
        profilePosts.map((post: IPostDoc) => post.populateUserActions(userId))
      );
    }
  ...
  paginateSearch: async (req, res) => {
    ...
    if (text) {
      ...
      if (userId) {
        await Promise.all(
          foundPosts.map((post: IPostDoc) => post.populateUserActions(userId))
        );
      }
  ...
  getUserPosts: async (req, res) => {
    ...
    if (userId) {
      await Promise.all(
        posts.map((post: IPostDoc) => post.populateUserActions(userId))
      );
    }
  ...
  makeRepost: async (req, res) => {
    ...
    if (postId && userId) {
      ...
      const repostExists = await Post.exists({
        repostOf: postId,
        user: userId,
      });
      if (repostExists) {

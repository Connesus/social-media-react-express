...
io.use(async (socket, next) => {
  ...
  // @ts-ignore
  const dbSession = sessionCollection.findOne({ _id: unsigned });
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
...
io.on("connection", async (socket) => {
  ...
  socket.on("fetchAllChatMessages", async (payload, ackFn) => {
    ...
    const allMessages = await Message.find({
      $or: [
        { sender: id, receiver: socket.id },
        { receiver: id, sender: socket.id },
      ],
    });
    ...
    app.use(
      session({
        ...
        store: MongoStore.create({
          // connect-mongo bug https://github.com/jdesboeufs/connect-mongo/issues/433
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore
          client: dbClient,
          dbName: "test",

...
export interface RequestStatus {
  id: string;
  status: STATUS.FAILED | STATUS.SUCCESS | STATUS.REQUEST | STATUS.IDLE;
  error?: string;
}
...
const isStartingRequestAction = (action: AnyAction): action is PayloadAction<SuccessPayload> => action.type.endsWith('REQUEST');
const isSuccessfulRequestAction = (action: AnyAction): action is PayloadAction<RequestPayload> => action.type.endsWith('SUCCESS');
const isFailedRequestAction = (action: AnyAction): action is PayloadAction<FailPayload> => action.type.endsWith('ERROR');
...
export const requestSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      requestAdapter.updateOne(state, {id: payload.id, changes: {status: STATUS.FAILED, error: payload.error}})
    ))
  }
})
